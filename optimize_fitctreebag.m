...
%  Mdl= fitrensemble(train_x_feature_label_norm,train_y_feature_label_norm,"NumLearningCycles",50,"LearnRate",1);  
     ...
     fprintf('开始超参数优化...\n');
     fprintf('优化范围: NumTrees [%d-%d], MinLeafSize [%d-%d]\n', LB(1), UB(1), LB(2), UB(2));
     fprintf('预计总实验数: %d (种群) × %d (迭代) = %d 次\n\n', pop, M, pop * M);
...
elseif strcmp( method_mti,'GDPSO�ƽ������ҸĽ�PSO')==1
    [fMin,Mdl,Convergence_curve,pos]=GDPSO_GDPSO(pop,M,LB,UB,nvars,fit_fitrensemble1,train_x_feature_label_norm, train_y_feature_label_norm, vaild_x_feature_label_norm, vaild_y_feature_label_norm);
        ...
        % 如果没有匹配到任何算法，显示错误信息并创建默认模型
        fprintf('警告: 未识别的优化算法: "%s"\n', method_mti);
        fprintf('使用默认参数创建模型...\n');
        % 使用默认参数创建模型
        ...
        pos = [50, 5];
        warning('未识别的优化算法，使用默认参数: NumTrees=50, MinLeafSize=5');
    end
     
     % 确保pos变量存在（处理不同算法的返回值差异）
     if ~exist('pos', 'var') || isempty(pos)
         ...
         elseif exist('bestX', 'var') && ~isempty(bestX)
             ...
             % 使用默认值
             pos = [50, 5];
             warning('无法获取最优参数位置，使用默认值: NumTrees=50, MinLeafSize=5');
         end
     end
     
     % 确保fMin变量存在（处理不同算法的返回值差异）
     if ~exist('fMin', 'var') || isempty(fMin)
         ...
         elseif exist('fitness', 'var') && ~isempty(fitness)
             ...
             % 使用默认值
             fMin = 0.5;
             warning('无法获取最优适应度，使用默认值: 0.5');
         end
     end
     
     % 确保Mdl变量存在（处理不同算法的返回值差异）
     if ~exist('Mdl', 'var') || isempty(Mdl)
         ...
         if exist('Mdl_best', 'var') && ~isempty(Mdl_best)
             ...
             % 使用默认参数创建一个模型
             warning('无法获取最优模型，使用默认参数创建模型: NumTrees=50, MinLeafSize=5');
             Mdl = TreeBagger(50, train_x_feature_label_norm, train_y_feature_label_norm, ...
                 'Method', 'classification', 'MinLeafSize', 5);
         end
     end
     
     % 确保Convergence_curve变量存在（处理不同算法的返回值差异）
     if ~exist('Convergence_curve', 'var') || isempty(Convergence_curve)
         ...
         if exist('ccurve', 'var') && ~isempty(ccurve)
             ...
             % 使用默认值（创建一个简单的收敛曲线）
             Convergence_curve = ones(1, M) * fMin;
             warning('无法获取收敛曲线，使用默认值');
         end
     end
     
     % 确保Mdl变量存在（处理不同算法的返回值差异）
     if ~exist('Mdl', 'var') || isempty(Mdl)
         % 如果Mdl不存在，使用默认参数创建一个模型
         warning('无法获取优化后的模型，使用默认参数创建模型');
         try
             Mdl = TreeBagger(50, train_x_feature_label_norm, train_y_feature_label_norm, ...
                 'Method', 'classification', 'MinLeafSize', 5);
         catch ME
             error('无法创建默认模型: %s', ME.message);
         end
     end
     ...
     fprintf('========================================\n');
     fprintf('优化算法: %s\n', method_mti);
     fprintf('最优参数: NumTrees=%d, MinLeafSize=%d\n', round(pos(1)), round(pos(2)));
     fprintf('最优适应度: %.4f\n', fMin);
     fprintf('========================================\n\n');
...
%    global train_x_feature_label_norm train_y_feature_label_norm vaild_x_feature_label_norm vaild_y_feature_label_norm
    ...
    % 显示当前实验信息（每10次实验显示一次，或第一次显示）
    if mod(exp_count, 10) == 1 || exp_count == 1
        num_trees = round(pop(1));
        min_leaf = round(pop(2));
        fprintf('  [实验 %d] 训练模型: NumTrees=%d, MinLeafSize=%d', exp_count, num_trees, min_leaf);
    end
...
fprintf('开始迭代优化 (共 %d 次迭代)...\n', itemax);
for ite=1:itemax
    % 显示迭代进度（每10%或每5次迭代显示一次）
    if mod(ite, max(1, floor(itemax/10))) == 0 || ite == 1 || ite == itemax
        fprintf('\n[迭代 %d/%d (%.1f%%)] ', ite, itemax, ite/itemax*100);
        fprintf('当前最优适应度: %.4f', fmin0);
        if ite > 1
...
% 显示迭代内的进度（每处理一定数量的个体后显示）
if mod(ite, max(1, floor(itemax/10))) == 0 || ite == 1 || ite == itemax
    fprintf('  完成迭代 %d: 最优适应度=%.4f, NumTrees=%d, MinLeafSize=%d\n', ...
